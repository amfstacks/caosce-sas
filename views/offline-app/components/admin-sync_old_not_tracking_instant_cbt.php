...
<script>
    ...
    function syncDashboardController() {
        return {
            ...
            async resetAttempt_old(studentId) {
                let student = this.mergedRoster.find(s => s.id === studentId);
...
async resetAttempt(studentId) {
        ...
        // 1. Remove the finalized submission record from localForage
        let dbRecords = await localforage.getItem('caosce_exam_records');
        if (dbRecords) {
            dbRecords = dbRecords.filter(r => r.student_id !== studentId);
            await localforage.setItem('caosce_exam_records', dbRecords);
        ...
        // 2. BULLETPROOF TIMER & PROGRESS RESET
        // We find and destroy any localStorage key starting with this student's ID, 
        // completely bypassing the need to know the exact station ID.
        let keysToRemove = [];
        ...
        keysToRemove.forEach(key => {
            ...
            console.log(`Cleared cached progress: ${key}`);
        });
