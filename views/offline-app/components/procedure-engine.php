<div x-data="examinerController()" x-init="initProcedure()" class="w-full h-full flex flex-col bg-slate-100 text-slate-800 font-sans antialiased absolute inset-0 overflow-hidden">
    ...
    <script>
        const TENANT_SLUG_PROCEDURE = '<?php echo CURRENT_TENANT_SLUG ?? "global"; ?>';
        ...
        function examinerController() {
            return {
                ...
                async initProcedure() {
                    ...
                    // Boot up if this is the active view initially
                    // await this.bootstrapComponent();

                    // // Re-run bootup anytime the SPA navigates back to this screen
                    // window.addEventListener('navigate', async (e) => {
                    //     if (e.detail === 'procedure') {
                    //         await this.bootstrapComponent();
                    //     }
                    // });
                ...
                async bootstrapComponent() {
                    this.isLoading = true;
                    this.activeStudent = null; // Reset to roster view 
                    this.searchQuery = '';

                    const authStr = sessionStorage.getItem('caosce_offline_auth');
                    if (!authStr) {
                        this.showToast("Unauthorized. Please log in.", "error");
                        // setTimeout(() => this.$dispatch('navigate', 'login'), 1000);
                        setTimeout(() => window.dispatchEvent(new CustomEvent('navigate', { detail: 'login' })), 1500);
                        return;
                        return;
                    }

                    const parsedAuth = JSON.parse(authStr);
                    // Prevent strictly anyone except Examiners from viewing this
                    if (parsedAuth.role !== 'examiner') {
                        this.showToast("Access Denied: Requires Examiner credentials.", "error");
                        setTimeout(() => window.dispatchEvent(new CustomEvent('navigate', { detail: 'login' })), 1500);
                        // return;
                        // setTimeout(() => this.$dispatch('navigate', 'login'), 1500);
                        return;
                    }
                ...
                getBaseApiUrl() {
                    ...
                    return TENANT_SLUG_PROCEDURE !== 'global' ? `${basePath}/${TENANT_SLUG_PROCEDURE}` : basePath;
                },
                ...
                async loadOfflinePayload() {
                    try {
                        ...
                        this.showToast("Critical Error: Missing offline data. Please sync device.", "error");
                        setTimeout(() => this.$dispatch('navigate', 'login'), 2000);
                    }
                },
                ...
                logout() {
                    if(confirm("Sign out of the Examiner Interface? You will need your PIN to re-enter.")) {
                        ...
                        // Use SPA dispatch instead of full page reload
